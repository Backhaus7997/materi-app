...
-   **Editor de código** recomendado:
    ...
    -   VS Code u otro editor de tu preferencia

-   **Cliente HTTP** para probar la API:

    -   Extensión **Thunder Client** (VS Code) o **Postman**
...
Reemplazá `<URL_DEL_REPO>` por la URL real de tu repositorio (HTTPS o
SSH).
...
Desde la raíz del proyecto (`materi/`):
...
Esto instala todas las dependencias necesarias del backend (Express,
Prisma, etc.).
...
Ejecutar las migraciones de Prisma para crear/actualizar la base de
datos `dev.db`:
...
Si el proyecto ya tiene migraciones existentes, Prisma las aplicará en
orden automáticamente.

**Opcional:** abrir Prisma Studio para inspeccionar la base de datos en
modo visual:
...
Con las dependencias instaladas, el `.env` creado y las migraciones
aplicadas:
...
Esto instala todas las dependencias del frontend (React, Vite, React
Query, etc.).
...
Con estos pasos cualquier desarrollador puede clonar el repositorio y
levantar **Materi** en modo desarrollo.
